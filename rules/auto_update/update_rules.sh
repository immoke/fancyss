...
get_cdn(){
	...
	jq --arg variable "${CURR_DATE}" '.cdn.date = $variable' ${RULE_FILE} | sponge ${RULE_FILE}
	jq --arg variable "${MD5_VALUE}" '.cdn.md5 = $variable' ${RULE_FILE} | sponge ${RULE_FILE}
	jq --arg variable "${LINE_COUN}" '.cdn.count = $variable' ${RULE_FILE} | sponge ${RULE_FILE}
}
...
make_version(){
	...
	do
	if [ -f $RULE_PATH/$filename ]; then
		line4=${filename%.*}

		line3=`md5sum $RULE_PATH/$filename | sed 's/ /\n/g'| sed -n 1p`
		...
		echo $line1" # "$line3" "$line4 >>${RULE_PATH}/version1
	fi
	done
